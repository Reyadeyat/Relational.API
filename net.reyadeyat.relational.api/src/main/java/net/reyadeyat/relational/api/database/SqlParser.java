...
public class SqlParser {
    ...
    final static private ArrayList<String> parse_sql(String ii) {
        String symbol = "+-*<>/,.='()\r\n\t ";
        String ignore = "\r\n\t ";
        Character sc = '\'';
        Character esc = '\"';
        ...
        z = 0;
        sm0 = sm = sm1 = "";
        //ii = "case (f164086 when 10 then 'Normal Vacation' when 17 then 'unpaid Vacation' else '')";
        //ii = "f161197 - (f161216 * f161214)";
        //ii = "(f161197 - (f161216 * f161214))";
    ...
    final static public StringBuffer informixt_to_mysql_parser(ArrayList<String> mm) throws Exception {
        int ml = mm.size();
        Boolean st = false, p = false;
        Boolean concatenate = false;
        String sm0 = "", sm = "", sm1 = mm.get(0);
        StringBuffer fisk = new StringBuffer().append(" "), ms = new StringBuffer();
        for (int i = 0; i < ml; i++) {
            sm = sm1;
    ...
    final static public StringBuffer informixt_to_sqlserver_parser(ArrayList<String> mm) throws Exception {
        int ml = mm.size();
        Boolean st = false, p = false;
        Boolean concatenate = false;
        String sm0 = "", sm = "", sm1 = mm.get(0);
    ...
    private static String sqlParserInformixToMySql(String strSql) {
        String newSql = "";
        try {
            newSql = informixt_to_mysql_parser(parse_sql(strSql.replace("\n", "")))
                    .toString();
        } catch (Exception e) {
...
//            Logger.writeLog(errMsg, "SqlParser", fileName);
    ...
    private static String sqlParserInformixToSqlServer(String strSql) {
        String newSql = "";
        try {
            newSql = informixt_to_sqlserver_parser(parse_sql(strSql.replace("\n", "")))
                    .toString();
        } catch (Exception e) {
            String errMsg = "Error in Sql : " + strSql + " >> Exception "
                    + e.getMessage();
...
String targetDataBase = "sqlserver";
            if (targetDataBase.equalsIgnoreCase("mysql")) {
                String strSqlNew = sqlParserInformixToMySql(strSql);
                return (strSqlNew);
            } else if (targetDataBase.equalsIgnoreCase("sqlserver")) {
                String strSqlNew = sqlParserInformixToSqlServer(strSql);
                return (strSqlNew);
            }
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.err.println(sqlConvert("select 'test' || case (mast.customer_type)"));
...
//	System.err.println(sqlConvert("select REPLACE(cde_hr_job.job_name_ar, '\', ' ') from cde_hr_job"));
        ...
        fileName = "/linux/im.sql";
        file = new File(fileName);
        fis = new FileInputStream(file);
        data = new byte[(int) file.length()];
        fis.read(data);
        fis.close();
        String informix_sql = new String(data, "UTF-8");
        ...
        System.out.println("------------------------------------------------------------------------------");
        long t1, t2;
        System.out.println("MySQL SQL Statement");
        System.out.println("------------------------------------------------------------------------------");
        t1 = System.nanoTime();
        System.out.println(SqlParser.informixt_to_mysql_parser(informix_sql.replace("\n", " ")).toString());
        t2 = System.nanoTime();
        System.out.println("------------------------------------------------------------------------------");
